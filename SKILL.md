...
从"技术交底 → 体系设计 → 划界 → 撰写 → 附图 → 溯源审核 → 提交底稿"的完整流水线。
本 skill 在真实的多件发明专利组合申报（6 件专利、36 张附图、约 6.5 万字申请文本）
中全流程打磨，所有规范、脚本与坑点均经实战验证。案例已匿名化，见
...
阶段1 交底整理 → 阶段2 体系设计 → 阶段3 在先划界 → 阶段4 正文撰写
     ...
     → 阶段8 提交后程序与期限管理（通知书接收/缴费/实审/公布的法定时限）
```
...
- 阶段 7-8（提交规范、回归校验、通知书接收下载与提交后期限管理）：读 [references/submission-checklist.md](references/submission-checklist.md)
...
`evals/evals.json` 含 6 组触发与输出评测（4 组触发 + 1 组 Output Contract 验收 +
1 组边界排除）。修改 description 或流程后，先手动过一遍 evals 再发布——
...
│   └── submission-checklist.md       # cponline 提交规范 + 通知书接收 + 提交后期限管理
├── scripts/
